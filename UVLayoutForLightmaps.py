...
def ensure_uv_set_on_shape(shape, uvset_name):
    ...
    if "map1" not in existing:
        print(
            "Error: 'map1' UV set not found on {}. Cannot create '{}'.".format(
                shape, uvset_name
            )
        )
        return False
    try:
        ...
        try:
            mel.eval(
                'polyUVSet -copy -uvSet "map1" -newUVSet "{0}" "{1}";'.format(
                    uvset_name, shape
                )
            )
        except Exception as e:
            print(
                "Error: Failed to create UV set '{}' for {}. ({})".format(
                    uvset_name, shape, e
                )
            )
            return False
    print(
        "Info: Created new UV set '{}' for {} (copied from 'map1').".format(
            uvset_name, shape
        )
    )
    return True
...
def set_current_uv_set(shape, uvset_name):
    ...
    try:
        ...
        try:
            ...
            print(
                "Error: Failed to set current UV set '{}' for {}. ({})".format(
                    uvset_name, shape, e
                )
            )
            return False
...
def layout_uvs_for_shape(shape, uvset_name, shell_padding, tile_padding, resolution):
    ...
    try:
        ...
        mel_cmd = 'u3dLayout -res {res} -scl 1 -spc {sp} -mar {tp} -box 0 1 0 1 "{obj}";'.format(
            sp=float(shell_padding / 1024),
            tp=float(tile_padding / 1024),
        ...
        try:
            ...
            print(
                "Error: Failed to run u3dLayout on {} for UV set '{}'. ({})".format(
                    shape, uvset_name, e
                )
            )
            return False
...
def run_layout_for_selection(uvset_name, shell_padding, tile_padding, resolution):
    ...
    for shape in mesh_shapes:
        ...
        if uvset_name == "map1":
            print(
                "Error: Operation on 'map1' is prohibited. Please use a different UV set name."
            )
            return
        if not ensure_uv_set_on_shape(shape, uvset_name):
            continue
        if not layout_uvs_for_shape(
            shape, uvset_name, shell_padding, tile_padding, resolution
        ):
            continue
...
def build_ui():
    ...
    uv_field = cmds.textField(
        text="lightmap",
        annotation="Name of the UV set to create/use (do not use 'map1').",
    )
