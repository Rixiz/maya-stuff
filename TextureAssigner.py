...
def create_texture_nodes_from_directory(directory, prefix, extension):
    ...
    for name, path in file_paths.items():
        ...
        cmds.setAttr(
            f"{file_node}.ignoreColorSpaceFileRules", True
        )  # ignore CS File Rule を有効にする
    ...
    if selected_shaders:
        ...
        shading_group = cmds.sets(
            renderable=True, noSurfaceShader=True, empty=True, name=f"{shader_node}SG"
        )
        cmds.connectAttr(
            f"{shader_node}.outColor", f"{shading_group}.surfaceShader", force=True
        )
    ...
    if renderer == "VRayMtl":
        ...
        if "Albedo" in file_nodes:
            cmds.connectAttr(
                f'{file_nodes["Albedo"]}.outColor',
                f"{shader_node}.diffuseColor",
                force=True,
            )
        if "Metalness" in file_nodes:
            cmds.connectAttr(
                f'{file_nodes["Metalness"]}.outAlpha',
                f"{shader_node}.metalness",
                force=True,
            )
        if "Roughness" in file_nodes:
            cmds.connectAttr(
                f'{file_nodes["Roughness"]}.outAlpha',
                f"{shader_node}.reflectionGlossiness",
                force=True,
            )
        ...
        if "Normal" in file_nodes:
            cmds.connectAttr(
                f'{file_nodes["Normal"]}.outColor', f"{shader_node}.bumpMap", force=True
            )
        if "Opacity" in file_nodes:
            cmds.connectAttr(
                f'{file_nodes["Opacity"]}.outColor',
                f"{shader_node}.opacityMap",
                force=True,
            )
        if "Translucency" in file_nodes:
            cmds.connectAttr(
                f'{file_nodes["Translucency"]}.outColor',
                f"{shader_node}.translucencyColor",
                force=True,
            )
    elif renderer == "aiStandardSurface":
        if "Albedo" in file_nodes:
        ...
        if "Roughness" in file_nodes:
            cmds.connectAttr(
                f'{file_nodes["Roughness"]}.outAlpha',
                f"{shader_node}.specularRoughness",
                force=True,
            )
            if cmds.checkBox("cb_invert_alpha", q=True, value=True):
                cmds.setAttr(f'{file_nodes["Roughness"]}.invert', 1)
        if "Normal" in file_nodes:
            ...
            cmds.connectAttr(
                f'{file_nodes["Normal"]}.outColor',
                f"{normal_map_node}.input",
                force=True,
            )
        ...
        if "Opacity" in file_nodes:
            cmds.connectAttr(
                f'{file_nodes["Opacity"]}.outColor',
                f"{shader_node}.opacity",
                force=True,
            )
        if "Translucency" in file_nodes:
            cmds.connectAttr(
                f'{file_nodes["Translucency"]}.outColor',
                f"{shader_node}.subsurfaceColor",
                force=True,
            )
...
def select_initial_albedo_file(*args):
    base_file = cmds.fileDialog2(
        ...
        fileMode=1,
        caption="Select First Texture File",
    )
    if base_file:
...
def create_texture_nodes(*args):
    ...
    for name in create_flags:
        if create_flags[name]:
            ...
            if file_path:
                ...
                create_flags[name] = (
                    False  # ファイルが選択されなかった場合はフラグをFalseに設定
                )
                cmds.warning("Please select at least one texture file.")
                return
    ...
    for name in create_flags:
        if create_flags[name]:
            ...
            cmds.connectAttr(f"{place2d_node}.outUV", f"{file_node}.uvCoord")
            cmds.connectAttr(
                f"{place2d_node}.outUvFilterSize", f"{file_node}.uvFilterSize"
            )
            if name in ["Metalness", "Roughness", "Opacity"]:
                cmds.setAttr(f"{file_node}.alphaIsLuminance", True)
            if name in [
                ...
                cmds.setAttr(f"{file_node}.uvTilingMode", 3)
            cmds.setAttr(
                f"{file_node}.fileTextureName", file_paths[name], type="string"
            )
            if name in ["Metalness", "Roughness", "Normal", "Opacity"]:
                cmds.setAttr(f"{file_node}.colorSpace", "Raw", type="string")
            cmds.setAttr(
    ...
    if renderer == "VRayMtl":
        ...
        if "Roughness" in file_nodes:
            ...
            if create_flags["Roughness"] and cmds.checkBox(
